...
class StatusIcon extends HTMLElement
  ...
  initialize: (@terminalView) ->
    @classList.add 'tokamak-terminal-status-icon'
  ...
  setupTooltip: ->
    ...
    onMouseEnter = (event) =>
      return if event.detail is 'tokamak-terminal'
      @updateTooltip()
  ...
  updateTooltip: ->
    ...
    @dispatchEvent(new CustomEvent('mouseenter', bubbles: true, detail: 'tokamak-terminal'))
...
module.exports = document.registerElement('tokamak-terminal-status-icon', prototype: StatusIcon.prototype, extends: 'li')
