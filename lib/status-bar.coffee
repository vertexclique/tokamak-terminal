...
TokamakTerminalView = require './view'
...
class StatusBar extends View
  ...
  @content: ->
    @div class: 'tokamak-terminal status-bar', tabindex: -1, =>
  ...
  initialize: (@statusBarProvider) ->
    ...
    @subscriptions.add atom.commands.add 'atom-workspace',
      'tokamak-terminal:new': => @newTerminalView()
      'tokamak-terminal:toggle': => @toggle()
      'tokamak-terminal:next': =>
        return unless @activeTerminal
        return if @activeTerminal.isAnimating()
        @activeTerminal.open() if @activeNextTerminalView()
      'tokamak-terminal:prev': =>
        return unless @activeTerminal
        return if @activeTerminal.isAnimating()
        @activeTerminal.open() if @activePrevTerminalView()
      'tokamak-terminal:close': => @destroyActiveTerm()
      'tokamak-terminal:close-all': => @closeAll()
      'tokamak-terminal:rename': => @runInActiveView (i) -> i.rename()
      'tokamak-terminal:insert-selected-text': => @runInActiveView (i) -> i.insertSelection()
      'tokamak-terminal:insert-text': => @runInActiveView (i) -> i.inputDialog()

    @subscriptions.add atom.commands.add '.xterm',
      'tokamak-terminal:paste': => @runInActiveView (i) -> i.paste()
      'tokamak-terminal:copy': => @runInActiveView (i) -> i.copy()

    @subscriptions.add atom.workspace.onDidChangeActivePaneItem (item) =>
      return unless item?

      if item.constructor.name is "TokamakTerminalView"
        setTimeout item.focus, 100
      else if item.constructor.name is "TextEditor"
        mapping = atom.config.get('tokamak-terminal.core.mapTerminalsTo')
        ...
        if prevTerminal != nextTerminal
          if not nextTerminal?
            if item.getTitle() isnt 'untitled'
              if atom.config.get('tokamak-terminal.core.mapTerminalsToAutoOpen')
    ...
    @statusContainer.on 'dragstart', '.tokamak-terminal-status-icon', @onDragStart
    @statusContainer.on 'dragend', '.tokamak-terminal-status-icon', @onDragEnd
    @statusContainer.on 'dragleave', @onDragLeave
    @statusContainer.on 'dragover', @onDragOver
    @statusContainer.on 'drop', @onDrop

    handleBlur = =>
      if terminal = TokamakTerminalView.getFocusedTerminal()
  ...
  registerContextMenu: ->
    @subscriptions.add atom.commands.add '.tokamak-terminal.status-bar',
      'tokamak-terminal:status-red': @setStatusColor
      'tokamak-terminal:status-orange': @setStatusColor
      'tokamak-terminal:status-yellow': @setStatusColor
      'tokamak-terminal:status-green': @setStatusColor
      'tokamak-terminal:status-blue': @setStatusColor
      'tokamak-terminal:status-purple': @setStatusColor
      'tokamak-terminal:status-pink': @setStatusColor
      'tokamak-terminal:status-cyan': @setStatusColor
      'tokamak-terminal:status-magenta': @setStatusColor
      'tokamak-terminal:status-default': @clearStatusColor
      'tokamak-terminal:context-close': (event) ->
        $(event.target).closest('.tokamak-terminal-status-icon')[0].terminalView.destroy()
      'tokamak-terminal:context-hide': (event) ->
        statusIcon = $(event.target).closest('.tokamak-terminal-status-icon')[0]
        statusIcon.terminalView.hide() if statusIcon.isActive()
      'tokamak-terminal:context-rename': (event) ->
        $(event.target).closest('.tokamak-terminal-status-icon')[0].rename()
  ...
  registerPaneSubscription: ->
    @subscriptions.add @paneSubscription = atom.workspace.observePanes (pane) =>
      ...
      tabBar.on 'dragstart', (event) ->
        return unless event.target.item?.constructor.name is 'TokamakTerminalView'
        event.originalEvent.dataTransfer.setData 'tokamak-terminal-tab', 'true'
  ...
  createTerminalView: (autoRun) ->
    ...
    switch atom.config.get('tokamak-terminal.core.workingDirectory')
    ...
    shell = atom.config.get 'tokamak-terminal.core.shell'
    shellArguments = atom.config.get 'tokamak-terminal.core.shellArguments'
    args = shellArguments.split(/\s+/g).filter (arg) -> arg

    statusIcon = new StatusIcon()
    tokamakTerminalView = new TokamakTerminalView(id, pwd, statusIcon, this, shell, args, autoRun)
    statusIcon.initialize(tokamakTerminalView)

    tokamakTerminalView.attach()

    @terminalViews.push tokamakTerminalView
    @statusContainer.append statusIcon
    return tokamakTerminalView
  ...
  setStatusColor: (event) ->
    ...
    color = atom.config.get("tokamak-terminal.iconColors.#{color}").toRGBAString()
    $(event.target).closest('.tokamak-terminal-status-icon').css 'color', color

  clearStatusColor: (event) ->
    $(event.target).closest('.tokamak-terminal-status-icon').css 'color', ''

  onDragStart: (event) =>
    event.originalEvent.dataTransfer.setData 'tokamak-terminal-panel', 'true'

    element = $(event.target).closest('.tokamak-terminal-status-icon')
  ...
  onDragOver: (event) =>
    ...
    unless event.originalEvent.dataTransfer.getData('tokamak-terminal') is 'true'
      return

    newDropTargetIndex = @getDropTargetIndex(event)
    return unless newDropTargetIndex?
    @removeDropTargetClasses()
    statusIcons = @statusContainer.children '.tokamak-terminal-status-icon'
  ...
  onDrop: (event) =>
    ...
    panelEvent = dataTransfer.getData('tokamak-terminal-panel') is 'true'
    tabEvent = dataTransfer.getData('tokamak-terminal-tab') is 'true'
  ...
  onDropTabBar: (event, pane) =>
    {dataTransfer} = event.originalEvent
    return unless dataTransfer.getData('tokamak-terminal-panel') is 'true'
  ...
  getDropTargetIndex: (event) ->
    ...
    statusIcons = @statusContainer.children('.tokamak-terminal-status-icon')
    element = target.closest('.tokamak-terminal-status-icon')
    ...
    else if element.next('.tokamak-terminal-status-icon').length > 0
      statusIcons.index(element.next('.tokamak-terminal-status-icon'))
    else
  ...
  getStatusIcons: ->
    @statusContainer.children('.tokamak-terminal-status-icon')
