...
module.exports =
class TokamakTerminalView extends View
  animating: false
  id: ''
  maximized: false
  opened: false
  ...
  @content: ->
    @div class: 'tokamak-terminal terminal-view', outlet: 'tokamakTerminalView', =>
  ...
  initialize: (@id, @pwd, @statusIcon, @statusBar, @shell, @args=[], @autoRun=[]) ->
    ...
    @prevHeight = atom.config.get('tokamak-terminal.style.defaultPanelHeight')
    ...
    @setAnimationSpeed()
    @subscriptions.add atom.config.onDidChange 'tokamak-terminal.style.animationSpeed', @setAnimationSpeed

    override = (event) ->
      return if event.originalEvent.dataTransfer.getData('tokamak-terminal') is 'true'
  ...
  setAnimationSpeed: =>
    @animationSpeed = atom.config.get('tokamak-terminal.style.animationSpeed')
  ...
  displayTerminal: ->
    ...
    @terminal = new Terminal {
      cursorBlink     : false
      scrollback      : atom.config.get 'tokamak-terminal.core.scrollback'
      cols, rows
    }
  ...
  attachListeners: ->
    @ptyProcess.on "tokamak-terminal:data", (data) =>
      @terminal.write data

    @ptyProcess.on "tokamak-terminal:exit", =>
      @destroy() if atom.config.get('tokamak-terminal.toggles.autoClose')
    ...
    @ptyProcess.on "tokamak-terminal:title", (title) =>
      @process = title
    ...
    @terminal.once "open", =>
      ...
      autoRunCommand = atom.config.get('tokamak-terminal.core.autoRunCommand')
  ...
  applyStyle: ->
    config = atom.config.get 'tokamak-terminal'
    ...
    @subscriptions.add atom.config.onDidChange 'editor.fontFamily', (event) =>
      ...
      @terminal.element.style.fontFamily = overrideFont or editorFont or defaultFont
    @subscriptions.add atom.config.onDidChange 'tokamak-terminal.style.fontFamily', (event) =>
    ...
    @subscriptions.add atom.config.onDidChange 'editor.fontSize', (event) =>
      ...
      @terminal.element.style.fontSize = "#{overrideFontSize or editorFontSize}px"
      @resizeTerminalToView()
    @subscriptions.add atom.config.onDidChange 'tokamak-terminal.style.fontSize', (event) =>
  ...
  insertSelection: ->
    ...
    runCommand = atom.config.get('tokamak-terminal.toggles.runInsertedText')
  ...
  getTitle: ->
    @statusIcon.getName() or "tokamak-terminal"
